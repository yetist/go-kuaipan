...
func (o *OrderedParams) String() string {
	result := ""
	for pos, key := range o.Keys() {
		if pos != 0 {
			result += "&"
		}
		result += fmt.Sprintf("%s=%s", key, o.Get(key))
	}
	return result
}
